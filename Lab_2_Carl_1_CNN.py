...
class CNNmodel(nn.Module):
    def __init__(self, filters, neurons_fully_connected_linear):
        super(CNNmodel, self).__init__()
        self.conv1 = nn.Conv2d(3, filters, kernel_size=3)
        self.pool = nn.MaxPool2d(2, 2)
        self.fc1 = nn.Linear(filters*13*13, neurons_fully_connected_linear) # 32 feature maps and 13x13 is the height and width of the maps, 128 is arbitrary
        self.fc1_dropout = nn.Dropout(dropout) # Dropout neurons
        self.fc2 = nn.Linear(neurons_fully_connected_linear, 1) # Single ouputs for binary classification
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x)))
        x = x.view(-1, filters*13*13)
        x = F.relu(self.fc1(x))
...
if __name__ == '__main__':
    ...
    epochs = 100 # Number of iterations through the CNN
    epochs_check = 10 # How often we check how the model is doing during training
    learning_rate = 0.001 # lr = learning rate (if error dosen't go down after a bunch of iterations (epox), lower our learning rate)
    neurons_fully_connected_linear = 128 # Amount of fully connected neurons in the local network
    dropout = 0.5 # The amount of neurons that will be dropped each iteration during training to minimize overfittning
    weight_decay = 0.003 # Stops some of the neurons from being to or not to activated to avoid overfitting
    filters = 32 # Number of filters maps in the convolution neural network
    ...
    CNNmodel = CNNmodel(filters=filters, neurons_fully_connected_linear=neurons_fully_connected_linear)
